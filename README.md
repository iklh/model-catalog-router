...
When adding or refreshing a Provider, automatic discovery calls its OpenAI-compatible `GET /v1/models` endpoint. A failed request can be retried, can return to the Base URL and API key prompts, or can fall back to manual entry. With ten or fewer discovered models, press Enter to select all or enter a complete selection such as `1,3-5`. With more than ten, the menu reports the total and asks whether to show all models; Enter defaults to not showing all. You can then search case-insensitively by a single keyword (spaces are ignored), select from the first ten matches, and repeat filtering, or enter models manually. Manual mode accepts model names without contacting the Provider or validating them. The quick model editor also accepts names containing `/` and `:`, such as `z-ai/glm-5.3-flash:free`; only the Provider prefix is removed during routing.
